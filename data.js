// api/data.js
// Single JSON store for all site data: products (multi-type), testimonials,
// faqs, orders, carts, site settings (which product is "hero" right now).
...
// Each type defines which fields are relevant for it. The admin
// panel reads this to render the right form per type, and the
// product page reads it to render the right layout per type.
// ───────────────────────────────────────────────────────────
const PRODUCT_TYPES = {
  workshop: {
    label: 'ورشة تدريبية',
    icon: '🎓',
    fields: ['duration', 'durationSub', 'format', 'formatSub', 'sessions', 'sessionsSub', 'outputs', 'extras']
  },
  book: {
    label: 'كتاب',
    icon: '📘',
    fields: ['pagesCount', 'fileFormat', 'language', 'tableOfContents', 'extras']
  },
  coaching: {
    label: 'جلسة كوتشينج فردية',
    icon: '🧭',
    fields: ['sessionDuration', 'sessionsCount', 'format', 'formatSub', 'outputs', 'extras']
  },
  video: {
    label: 'فيديو / كورس مسجل',
    icon: '🎬',
    fields: ['videoDuration', 'lessonsCount', 'accessPeriod', 'outputs', 'extras']
  },
  other: {
    label: 'منتج آخر',
    icon: '📦',
    fields: ['customField1Label', 'customField1Value', 'customField2Label', 'customField2Value', 'outputs', 'extras']
  }
};
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    // Include the type definitions so the admin panel and product
    // pages can render the right fields without duplicating this logic.
    res.status(200).json({ ...data, productTypes: PRODUCT_TYPES });
    return;
  }
  ...
  if (req.method === 'POST') {
    try {
      ...
      delete body.productTypes; // never persist this, it's derived/static
      const current = await getCurrentData();
      const updated = { ...current, ...body };

      // Enforce: only one product can be the "hero" at a time.
      if (body.products) {
        const heroId = updated.products.find(p => p.isHero)?.id;
        if (heroId) {
          updated.products = updated.products.map(p => ({ ...p, isHero: p.id === heroId }));
        }
      }

      await saveData(updated);
      res.status(200).json({ ok: true, data: { ...updated, productTypes: PRODUCT_TYPES } });
    } catch (err) {
